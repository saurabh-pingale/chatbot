"""Add variant_quantity to products

Revision ID: 59bfe9b460be
Revises: <new_revision_id>
Create Date: 2025-10-01 14:38:03.636716
...
def downgrade() -> None:
    ...
    sa.Column('type', postgresql.ENUM('STANDARD', 'ANALYTICS', name='buckettype', schema='storage'), server_default=sa.text("'ANALYTICS'::storage.buckettype"), autoincrement=False, nullable=False),
    ...
    sa.Column('level', sa.INTEGER(), sa.Computed('storage.get_level(name)', persisted=True), autoincrement=False, nullable=False),
    sa.Column('created_at', postgresql.TIMESTAMP(timezone=True), server_default=sa.text('now()'), autoincrement=False, nullable=True),
    sa.Column('updated_at', postgresql.TIMESTAMP(timezone=True), server_default=sa.text('now()'), autoincrement=False, nullable=True),
    sa.ForeignKeyConstraint(['bucket_id'], ['storage.buckets.id'], name=op.f('prefixes_bucketId_fkey')),
    sa.PrimaryKeyConstraint('bucket_id', 'level', 'name', name=op.f('prefixes_pkey')),
    schema='storage'
    )
    op.create_index(op.f('idx_prefixes_lower_name'), 'prefixes', ['bucket_id', 'level', sa.literal_column("((string_to_array(name, '/'::text))[level])"), sa.literal_column('lower(name)')], unique=False, schema='storage')
    ...
    sa.Column('owner_id', sa.TEXT(), autoincrement=False, nullable=True),
    sa.Column('type', postgresql.ENUM('STANDARD', 'ANALYTICS', name='buckettype', schema='storage'), server_default=sa.text("'STANDARD'::storage.buckettype"), autoincrement=False, nullable=False),
    ...
    sa.UniqueConstraint('client_id', name=op.f('oauth_clients_client_id_key'), postgresql_include=[], postgresql_nulls_not_distinct=False),
    schema='auth'
    )
    op.create_index(op.f('oauth_clients_deleted_at_idx'), 'oauth_clients', ['deleted_at'], unique=False, schema='auth')
    op.create_index(op.f('oauth_clients_client_id_idx'), 'oauth_clients', ['client_id'], unique=False, schema='auth')
    ...
    op.create_index(op.f('objects_bucket_id_level_idx'), 'objects', ['bucket_id', 'level', 'name'], unique=True, schema='storage')
    op.create_index(op.f('name_prefix_search'), 'objects', ['name'], unique=False, schema='storage')
    op.create_index(op.f('idx_objects_lower_name'), 'objects', [sa.literal_column('(path_tokens[level])'), sa.literal_column('lower(name)'), 'bucket_id', 'level'], unique=False, schema='storage')
    op.create_index(op.f('idx_objects_bucket_id_name'), 'objects', ['bucket_id', 'name'], unique=False, schema='storage')
    op.create_index(op.f('idx_name_bucket_level_unique'), 'objects', ['name', 'bucket_id', 'level'], unique=True, schema='storage')
