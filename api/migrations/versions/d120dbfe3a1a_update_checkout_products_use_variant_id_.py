"""update checkout_products: use variant_id instead of product_id

Revision ID: d120dbfe3a1a
Revises: d167c3276101
Create Date: 2025-07-22 17:01:29.949025
...
revision: str = 'd120dbfe3a1a'
down_revision: Union[str, None] = 'd167c3276101'
...
def upgrade() -> None:
    ...
    # Drop otps table/index only if they exist (may already be removed by 655bc24dcb8d_remove_otp_model)
    conn = op.get_bind()
    conn.execute(sa.text("DROP INDEX IF EXISTS ix_otps_email"))
    conn.execute(sa.text("DROP TABLE IF EXISTS otps CASCADE"))
    op.add_column('checkout_products', sa.Column('variant_id', sa.BigInteger(), nullable=False))
    op.drop_constraint(op.f('checkout_products_product_id_fkey'), 'checkout_products', type_='foreignkey')
    op.drop_column('checkout_products', 'product_id')
    op.add_column('products', sa.Column('variant_id', sa.BigInteger(), nullable=True))
    op.create_unique_constraint('uq_products_variant_id', 'products', ['variant_id'])
...
def downgrade() -> None:
    """Downgrade schema."""
    # ### commands auto generated by Alembic - please adjust! ###
    op.drop_constraint('uq_products_variant_id', 'products', type_='unique')
    op.drop_constraint(None, 'products', type_='unique')
    op.drop_column('products', 'variant_id')
    op.add_column('checkout_products', sa.Column('product_id', sa.BIGINT(), autoincrement=False, nullable=False))
    op.create_foreign_key(op.f('checkout_products_product_id_fkey'), 'checkout_products', 'products', ['product_id'], ['id'])
    op.drop_column('checkout_products', 'variant_id')
