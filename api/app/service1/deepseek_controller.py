...
from app.services.LLM_service import create_deepseek_prompt, generate_llm_response
from utils.shopify_proxy_utils import verify_app_proxy_signature
from utils.shopify_header_utils import (
    get_shopify_session_for_training,
    get_shop_namespace,
)
...
@router.post("")
async def endpoint(request: Request, body):
    ...
    try:
        if body.isTrainingPage:
            session = await get_shopify_session_for_training(
                request, is_training_page=body.isTrainingPage
            )
            ...
            if body.messages:
                ...
                try:
                    ...
                    response = await handle_product_embeddings_from_json(
                        custom_input_to_json_data, namespace=shopify_store
                    )
                    return {"answer": response}
                except json.JSONDecodeError:
                    return {
                        "answer": "Please provide a valid JSON input to train the LLM."
                    }

            await fetch_generate_store_product_embeddings(
        ...
        query_results = await query_embeddings(
        ...
        products = [
            product for product in products if product["title"] and product["url"]
        ]
